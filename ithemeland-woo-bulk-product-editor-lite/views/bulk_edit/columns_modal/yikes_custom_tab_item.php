...
$wcbel_tab_title = (!empty($tab['title'])) ? $tab['title'] : esc_html__('Tab title', 'ithemeland-woo-bulk-product-editor-lite');
$wcbel_tab_content = (!empty($tab['content'])) ? wp_kses($tab['content'], Sanitizer::allowed_html()) : '';
$wcbel_unique_id = (!empty($tab_unique_id)) ? $tab_unique_id : 'editor-' . wp_rand(100, 999);
?>

<div class="wcbe-yikes-tab-item" <?php echo (!empty($wcbel_duplicate_item)) ? 'id="duplicate-item"' : ''; ?>>
    <div class="wcbe-yikes-tab-item-header">
        <strong class="wcbe-yikes-tab-item-header-title"><?php echo esc_html($wcbel_tab_title); ?></strong>
    ...
    <div class="wcbe-yikes-tab-item-body">
        ...
        <div class="wcbe-yikes-tab-title">
            ...
            <input type="text" placeholder="<?php esc_html_e('Tab title ...', 'ithemeland-woo-bulk-product-editor-lite'); ?>" value="<?php echo esc_attr($wcbel_tab_title); ?>">
        </div>
        <div class="wcbe-yikes-tab-content" data-id="<?php echo esc_attr($wcbel_unique_id); ?>">
            <label><?php esc_html_e('Tab content', 'ithemeland-woo-bulk-product-editor-lite'); ?></label>
            <?php if (!empty($wcbel_duplicate_item)) : ?>
                <textarea placeholder="<?php esc_html_e('Content ...', 'ithemeland-woo-bulk-product-editor-lite'); ?>"></textarea>
            <?php
            else :
                wp_editor($wcbel_tab_content, $wcbel_unique_id);
            endif;
            ?>
        </div>
    </div>
